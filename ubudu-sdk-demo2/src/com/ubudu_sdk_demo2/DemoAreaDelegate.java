...
public class DemoAreaDelegate implements UbuduAreaDelegate {
	...
	public void areaEntered(final UbuduGeofence fence) {
		...
		refresh.post(new Runnable() {
			public void run() {
				mOutput.printf(
						"geofence entered id=%3s lat=%11.7f lng=%11.7f radius=%5.2f\n  name=\"%s\"\n",
						fence.id(), fence.centerLatitude(), fence.centerLongitude(),
						fence.radius(), fence.name());
			}
		});
	...
	public void areaExited(final UbuduGeofence fence) {
		...
		refresh.post(new Runnable() {
			public void run() {
				mOutput.printf(
						"geofence exited  id=%3s lat=%11.7f lng=%11.7f radius=%5.2f\n  name=\"%s\"\n",
						fence.id(), fence.centerLatitude(), fence.centerLongitude(),
						fence.radius(), fence.name());
			}
		});
	...
	public void areaEntered(final UbuduBeaconRegion beacon) {
		...
		refresh.post(new Runnable() {
			public void run() {
				mOutput.printf(
						"beacon detected   in   region id=%3s \n  UUID=%s\n  major=%s minor=%s\n  name=\"%s\" \n",
						beacon.id(), beacon.proximityUUID(), beacon.major(), beacon.minor(), beacon
								.name());
			}
	...
	public void areaExited(final UbuduBeaconRegion beacon) {
		...
		refresh.post(new Runnable() {
			public void run() {
				mOutput.printf(
						"beacon disappeared from region id=%3s \n  UUID=%s\n  major=%s minor=%s\n  name=\"%s\" \n",
						beacon.id(), beacon.proximityUUID(), beacon.major(), beacon.minor(), beacon
								.name());
			}
		});
		...
		refresh.post(new Runnable() {
			public void run() {
				mOutput.printf("geofence rule fired %-7s id=%3s \n  name=\"%s\")\n", ((event
						.eventKind() == UbuduEvent.ENTERED) ? "entered" : "exited"), event.area()
						.id(), event.area().name());
			}
		});
		...
		refresh.post(new Runnable() {
			public void run() {
				mOutput.printf("geofence notify user for event %7s id=%3s\n  name=\"%s\"\n",
						((event.eventKind() == UbuduEvent.ENTERED) ? "entered" : "exited"), event
								.area().id(), event.area().name());
			}
		});
		return true;
